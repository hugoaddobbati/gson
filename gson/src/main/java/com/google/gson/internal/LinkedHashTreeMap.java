...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
	 ...
	 *
	 * @throws ClassCastException if {@code key} and the tree's keys aren't mutually
	 *                            comparable.
	 */
	...
	void removeInternal(Node<K, V> node, boolean unlink) {
		...
		if (left != null && right != null) {
			...
			left = node.left;
			if (left != null) {
				leftHeight = leftNotNull(node, left, adjacent);
			}
			int rightHeight = 0;
			right = node.right;
			if (right != null) {
				rightHeight = rightNotNull(node, right, adjacent);
			}
	...
	private int rightNotNull(Node<K, V> node, Node<K, V> right, Node<K, V> adjacent) {
		int rightHeight;
	...
	private int leftNotNull(Node<K, V> node, Node<K, V> left, Node<K, V> adjacent) {
		int leftHeight;
	 ...
	 *
	 * @param insert true if the node was unbalanced by an insert; false if it was
	 *               by a removal.
	 */
